...
class ForwardProblemSolver:
    ...
    def scatterer_independent_data(self):
        ...
        direct_power = self.model.get_power_from_field(direct_field, 0)
    ...
    def add_noise(self, total_field):
        nl = 0
        Ns = 40
        Ni = 40
        rand_real = np.random.randn(Ns - 1, Ni)
        E_Gaussian = (1 / np.sqrt(2)) * np.sqrt(1 / Ns / Ni) * np.linalg.norm(total_field, 'fro') * nl * rand_real
        return E_Gaussian
    ...
    def scatterer_dependent_data(self, direct_field):
        ...
        noise = self.add_noise(total_field)
        total_power = self.model.get_power_from_field(total_field, noise)
